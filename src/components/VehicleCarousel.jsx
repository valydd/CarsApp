...
export const VehicleCarousel = ({
  ...
  return (
    <section className="mb-3.5">
      {/* Compact Top Bar: [Vehicule] + [Plate / Badge] on Left, [Backup] on Right */}
      <div className="flex items-center justify-between gap-1 xs:gap-2 px-0.5 w-full">
        ...
        <div className="flex items-center gap-1.5 xs:gap-2 min-w-0">
          ...
          <button
            ...
            className={`inline-flex items-center gap-1 px-2.5 py-1.5 rounded-xl border text-[11px] font-black shadow-2xs active:scale-95 transition-all shrink-0 ${
              !isAllSelected
          ...
          {isAllSelected && (
            <span className="text-[10.5px] xs:text-[11px] font-bold text-slate-500 dark:text-slate-400 truncate">
          ...
          {singleVehicle && (
            <div className="inline-flex items-center gap-1 bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 px-1.5 py-0.5 rounded-lg text-[10.5px] xs:text-[11px] font-mono font-black text-slate-900 dark:text-slate-100 shadow-2xs whitespace-nowrap shrink-0">
              <span className="text-[8px] text-blue-500 font-bold leading-none">RO</span>
          ...
          {!isAllSelected && !singleVehicle && (
            <div className="inline-flex items-center gap-1 bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-500/40 px-2 py-0.5 rounded-lg text-[10.5px] xs:text-[11px] font-extrabold text-emerald-700 dark:text-emerald-300 shadow-2xs whitespace-nowrap shrink-0">
              <span>
                {lang === 'en' 
                  ? `${selectedVehicleIds.length} of ${vehicles.length} vehicles` 
                  : `${selectedVehicleIds.length} din ${vehicles.length} mașini`}
              </span>
              <button
                type="button"
                onClick={(e) => {
                ...
                className="text-emerald-600 dark:text-emerald-400 hover:text-rose-500 p-0.5 rounded transition-colors ml-0.5"
                title={lang === 'en' ? "Select all" : "Selectează toate"}
              >
                <X className="w-3 h-3" />
              </button>
        ...
        {/* Right side: QR / Connect and Backup buttons */}
        <div className="flex items-center gap-1 xs:gap-1.5 ml-auto shrink-0">
          {onOpenConnect && (
            <button
              type="button"
              onClick={onOpenConnect}
              className="flex items-center gap-1 text-[10.5px] xs:text-[11px] font-extrabold text-blue-700 dark:text-blue-300 hover:text-blue-800 dark:hover:text-blue-200 bg-blue-50 dark:bg-blue-950/40 border border-blue-200 dark:border-blue-900/50 hover:bg-blue-100 dark:hover:bg-blue-900/50 transition-colors py-1 px-2 xs:px-2.5 rounded-xl shrink-0 shadow-2xs whitespace-nowrap cursor-pointer active:scale-95"
          ...
          {onOpenExport && (
            <button
              ...
              className="flex items-center gap-1 text-[10.5px] xs:text-[11px] font-bold text-slate-700 dark:text-slate-200 hover:text-emerald-600 dark:hover:text-emerald-400 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors py-1 px-2 xs:px-2.5 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shrink-0 shadow-2xs whitespace-nowrap cursor-pointer active:scale-95"
